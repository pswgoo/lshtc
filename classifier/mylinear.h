...
public:
	...
	static int PraseFile(const char* const fileName, std::vector<double> &labels, std::vector<Feature>&features);

	static int PraseRanksvmFile(const char* const fileName, std::vector<double> &labels, std::vector<int> &queries, std::vector<Feature>&features);
	...
	//featureSize 是x数组第一维的大小,即feature个数
	static int SaveFeatureNode(feature_node** x, size_t featureSize, const std::string& fileName);

	//featureSize 是x数组第一维的大小,即feature个数
	static int LoadFeatureNode(feature_node** &x, feature_node* &x_space, size_t& featureSize, const std::string& fileName);
